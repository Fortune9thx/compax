...
| VaultManager | `0xb751aF3959216f5A9abe149CD575Eb1eE873F06e` | redeployed twice more same day - once fixing an `on='finalized'` delivery issue, once extending authoritative sourcing to `resolve_movement()` - see SECURITY.md |
...
| CreditLine | `0x09e8d5C9DF87c61198a837fd9ffC6b08EfBf6759` | redeployed once more same day, extending authoritative sourcing to `resolve_default()` - see SECURITY.md |
...
- **VaultManager**: mandate scope is a transparent, deterministic risk-tier rule (not an LLM decision - running a fixed lookup through consensus would be decorative), and `move_to_prediction` is still rejected on-chain (`FINISHED_WITH_ERROR`) when a vault's risk tier doesn't permit it. The real Intelligent Contract behavior lives in `resolve_movement()`: a depositor challenges a specific capital movement against the vault's stated objective (optionally with a real evidence URL, fetched live and weighed as primary evidence), and five validators independently adjudicate compliant vs. violation. Capital movement itself funds the target instrument **directly** via a cross-contract call carrying value - verified live: a vault funded a real, separately-opened credit line with the vault owner (not `VaultManager` itself) landing as `lender` of record, confirmed both via the line's own state and via `VaultManager`'s balance staying flat through a full repay cycle. A challenge with a real evidence URL (the project's own README) resolved with reasoning that cited specific fetched content, not boilerplate.
